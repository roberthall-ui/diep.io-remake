...
function startShooter() {
  if (shooterStarted) return; // prevent multiple instances
  shooterStarted = true;
  ...
  document.addEventListener("keydown", e => {
    if (e.key === " ") {
      ...
      bullets.push({ x: player.x, y: player.y, r: 4, dx: Math.cos(angle)*5, dy: Math.sin(angle)*5 });
    }
  });

  function spawnTarget() {
    targets.push({ x: Math.random()*canvas.width, y: Math.random()*canvas.height, size: 15, health: 3, dx:(Math.random()-0.5)*2, dy:(Math.random()-0.5)*2 });
  }
    ...
    bullets.forEach((b,bi)=>{
      targets.forEach((t,ti)=>{
        ...
        if(Math.sqrt(dx*dx+dy*dy)<t.size){
          bullets.splice(bi,1);
          t.health--;
          for(let i=0;i<5;i++) particles.push({x:b.x,y:b.y,dx:(Math.random()-0.5)*4,dy:(Math.random()-0.5)*4,life:20});
  ...
  function draw() {
    ...
    ctx.fillStyle="cyan"; ctx.beginPath(); ctx.arc(0,0,player.size,0,Math.PI*2); ctx.fill(); ctx.restore();

    ctx.fillStyle="yellow"; bullets.forEach(b=>{ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill();});
    targets.forEach(t=>{
    ...
    ctx.fillStyle="orange"; particles.forEach(p=>{ctx.beginPath();ctx.arc(p.x,p.y,2,0,Math.PI*2);ctx.fill();});
